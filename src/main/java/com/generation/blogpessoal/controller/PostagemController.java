...
São os recursos Optional e Expressões Lambda. */
    ...
    public ResponseEntity<Postagem> post(@Valid @RequestBody Postagem postagem){
        ...
        /*Executa o Método padrão da Interface JpaRepository (save(postagem)) e retorna o HTTP Status CREATED🡪201 se o Objeto
                foi persistido no Banco de dados. */
    }
    ...
    @PutMapping
    /*Linha 53: a anotação @PutMapping indica que o Método put(Postagem postagem), responderá a todas as requisições do tipo HTTP PUT,
    enviadas no endereço http://localhost:8080/postagens */
    ...
    public ResponseEntity<Postagem> put(@Valid @RequestBody Postagem postagem){
        return postagemRepository.findById(postagem.getId())
                .map(resposta -> ResponseEntity.status(HttpStatus.OK)
                        .body(postagemRepository.save(postagem)))
                        .orElse(ResponseEntity.status(HttpStatus.NOT_FOUND).build());
        /*. orElse(ResponseEntity.notFound().build());: Se o Objeto Postagem não for encontrado (Nulo) pelo Método findById(id),
        será retornado o HTTP Status NOT FOUND 🡪 404 (Não Encontrado!). O Método build() constrói a Resposta com o HTTP Status retornado. */
    }
